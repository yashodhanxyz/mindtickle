...
## Sidebar hover refinement — 11 September 2026

Sidebar search, New chat, menu context, conversation rows, overflow actions, and popover menu items now use the same neutral hover family and short token-based color transition. Hover rules are limited to hover-capable fine pointers. The selected conversation retains its accent background when hovered, preventing the earlier state flicker. Overflow actions are quiet at rest, appear on hover or `:focus-within`, and stay visible on touch devices.

Live browser checks confirmed the menu hover surface, stable selected-chat background, overflow-action reveal, and keyboard Tab access to the same action. Global focus-visible outlines remain unchanged. Static host navigation retains its default cursor because those destinations are outside the implemented assignment flow. Screenshots and notes are in [the sidebar hover review](../docs/design/sidebar-hover/README.md).
