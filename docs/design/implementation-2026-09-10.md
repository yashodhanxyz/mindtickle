...
Run the app using the repository README. Choose **Floating chat** or **Third column** above the supplied initial question, then submit it. The first question is deliberately preset to the assignment's fixed assessment. Follow-up text is editable inside the conversation.
...
- At widths of 760 CSS pixels or less, both become a full-height conversation. The column also takes this form below 1,100 pixels; floating chat stays available on smaller desktops. Close returns to the host; floating mode also offers the minimized bar.
